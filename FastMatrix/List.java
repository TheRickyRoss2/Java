...
//	Your list module for this project will be a bi-directional queue that includes a â€œcursorâ€� to be used for iteration.
//	Think of the cursor as highlighting or underscoring a distinguished element in the list. Note that it is a valid state
//	for this ADT to have no distinguished element, i.e. the cursor may be undefined or â€œoffâ€�, which is in fact its
//	default state. Thus the set of â€œmathematical structuresâ€� for this ADT consists of all finite sequences of integers
//	in which at most one element is underscored. A list has two ends referred to as â€œfrontâ€� and â€œbackâ€� respectively.
...
public class List {
	...
	private class Node {
		...
		public boolean equals(Object x){
			if( x instanceof Node){
				Node n = (Node) x;
				if(!this.data.equals(n.data)) return false;
			}
			return true;
		}
		@Override
	...
	Object get() {
		if (this.index() < 0)
			throw new RuntimeException("List.java: get(): Index undefined");
		if (this.length() <= 0)
			throw new RuntimeException("List.java: get(): Empty List");
	...
	public boolean equals(Object x) {
		if (x instanceof List) {
			...
			for(this.moveFront(), L.moveFront(); this.index>=0;this.moveNext(), L.moveNext()){
